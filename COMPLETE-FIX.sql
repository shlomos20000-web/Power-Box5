-- COMPLETE FIX FOR DATABASE ERRORS
...
CREATE POLICY "public_read_offer_pricing" ON offer_pricing FOR SELECT USING (true);
CREATE POLICY "public_read_customer_reviews" ON customer_reviews FOR SELECT USING (true);
CREATE POLICY "public_read_footer" ON footer FOR SELECT USING (true);
CREATE POLICY "public_read_product_gallery" ON product_gallery FOR SELECT USING (true);
CREATE POLICY "public_read_trust_section" ON trust_section FOR SELECT USING (true);
...
-- Success message
SELECT 
    'SUCCESS: All 5 tables created with default data!' as status,
    'Your website errors should now be fixed. Refresh the page!' as instruction;
