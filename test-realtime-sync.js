...
async function testDatabaseConnection() {
  ...
  try {
    ...
    try {
      ...
      if (updateError) {
        console.log('⚠️  Real-time test update failed (table might not exist):', updateError.message);
      } else {
...
testDatabaseConnection().then(success => {
  ...
  if (!success) {
    console.log('\n📋 Next steps:');
    console.log('1. Run the create-supabase-tables.sql script in your Supabase dashboard');
